...
> (earmarked, no rows). Cross-phase order: ~~P3-X2~~ (done 2026-07-30) → P3-F3
> (broken-build class, found the same day) → P3.5-E3 → P6-4..AUDIT → P7 →
...
| P3-F3 | fixture | **`plane_audio_test` intermittent heap-corruption SIGABRT at HEAD** (found 2026-07-30 gating P3-X2; PRE-EXISTING — measured 9/30 aborts on HEAD 708365c with the P3-X2 diff stashed, similar rate with it, so not that row's regression): malloc `free_list_checksum_botch` surfacing under a later `juce::var` HeapBlock alloc — a write-after-free/overflow somewhere earlier in the run, dying with NO output. Repro: loop `build/shell/plane_audio_test` (~1 in 4); crash reports in `~/Library/Logs/DiagnosticReports/plane_audio_test-*.ips` | todo (broken-build class — take FIRST per §11) | The P3-F1 play: TSan/ASan first, then read the corpse. ASan is the sharper tool for heap corruption (`-fsanitize=address` on `wz_sl_*` + the test). Suspect surface is the test's own harness or the world/command tiers it drives (`shell/tools/plane_audio_test.cpp`, links wz_sl_command/world/engine/record) — the corruption predates every P3-X2 engine edit |
...
>
> **Global order across phases:** ~~P3-X2~~ (done 2026-07-30) → **P3-F3** (broken-build
> class, §11 priority) → **P3.5-E3** (hoisted defect) → P6-4 → P6-5 → P6-6 → P6-AUDIT →
> P7 → P8 → P9 → (P10). P3.5-E1 stays user-fed in parallel.
>
