...
> and P3-X5. X4 blocks trustworthy real-host walks.
>
...
| P3-X4 | build | **`webdist:check` is RED at HEAD — the shell ships a stale bundle** (found 2026-07-29 running the full gate suite for the plan-adjustment commit). The gate's own words: "the JUCE shell ships the committed `webdist/` bundle and never builds the web UI itself, so a stale bundle means the app runs old UI against new engine/protocol code." Consequence for the four rules: **every pending real-host walk (P3-SES-2, P3.5-AUDIT's gate list) is walking OLD UI** until the bundle is regenerated. Fix is mechanical — `cd web && npm run bundle`, commit `webdist/` — but it is its OWN commit, never folded into a feature | todo (do before offering any real-host walk) | the hash closure is `src` + `protocol` + index.html/vite/tsconfig/package(-lock); fixtures and `scripts/` are deliberately excluded, so this is genuine source drift, not fixture noise |
