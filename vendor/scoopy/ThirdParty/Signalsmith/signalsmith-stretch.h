...
struct SignalsmithStretch {
	...
	// [ScoopyLoops local patch] Phase character for extreme stretches (beyond
	// maxCleanStretch), BIPOLAR -1..+1:
	//   +1 = stock behaviour (random per-bin time factors -> diffuse/airy freeze)
	//    0 = fully coherent phase propagation at the TRUE time factor (the same grain
	//        loops in lockstep -> periodic comb/metallic/robotic freeze)
	//   -1 = all bins advance at the clean-stretch rate (maxCleanStretch) regardless of
	//        the true factor -> the frozen spectrum ROLLS forward as a locked drone
	//        instead of buzzing statically.
	// Values morph continuously within each half. Plain member write - RT-safe.
	void setPhaseChaos(Sample chaosBipolar) {
		phaseChaos = std::min<Sample>(1, std::max<Sample>(-1, chaosBipolar));
	}

	// [ScoopyLoops local patch] Spectral blur, 0..1: a symmetric per-bin dispersion
	// (+- blur*maxCleanStretch) of the phase-propagation rate that works at ANY stretch
	// ratio - INCLUDING unity/clean ratios, where phaseChaos has no effect because the
	// stock random path only opens beyond maxCleanStretch. At clean ratios blur opens
	// that path itself: 0 = stock (bit-identical RNG consumption), small = chorus-like
	// spectral diffusion whose grain size follows the analysis window, 1 = dense
	// smearing wash. Interactions: at clean ratios phaseChaos < 0 slides the dispersion
	// centre toward the clean-stretch rate so the blur cloud ROLLS forward (chaos >= 0
	// is inert there); at extreme stretch blur adds width on top of the chaos
	// character. Plain member write - RT-safe.
	void setPhaseBlur(Sample blur01) {
		phaseBlur = std::min<Sample>(1, std::max<Sample>(0, blur01));
	}

	// [ScoopyLoops local patch] Low-band map tracking: when enabled AND a custom freq map
	// is installed, the spectrum below the LOWEST detected peak follows the map (sampled
	// at octave-spaced synthetic anchor peaks) instead of the stock rigid unit-gradient
	// translate {b + bottomOffset, 1}. Lets a low-weighted warp actually move bass content
	// whose only representation is the sub-lowest-peak fill. false (default) =
	// bit-identical to stock. Plain member write - RT-safe.
	void setFreqMapLowFill(bool enabled) {
		freqMapLowFill = enabled;
	}
...
private:
	...
	Sample phaseChaos = 1; // [ScoopyLoops local patch] see setPhaseChaos()
	Sample phaseBlur = 0;  // [ScoopyLoops local patch] see setPhaseBlur()
	bool freqMapLowFill = false; // [ScoopyLoops local patch] see setFreqMapLowFill()
	...
	void processSpectrum(size_t step) {
		...
		// [ScoopyLoops local patch] Unified per-bin phase-propagation rate: phaseChaos
		// shapes the character where the stock random path is open (extreme stretch),
		// phaseBlur adds a symmetric +-blur*maxCleanStretch dispersion at ANY ratio
		// (opening the random path itself at clean ratios, where chaos < 0 also biases
		// the centre so the blur cloud rolls forward). blur == 0 leaves every branch
		// and every RNG draw identical to the pre-blur build. NOTE: timeFactorDist has
		// reversed (invalid) bounds when !randomTimeFactor, so the clean-ratio path
		// must only ever draw from blurDist.
		std::uniform_real_distribution<Sample> blurDist(-phaseBlur*maxCleanStretch, phaseBlur*maxCleanStretch);
		auto binTimeFactorFor = [&]() -> Sample {
			if (randomTimeFactor) {
				Sample base = (phaseChaos >= 0)
					? timeFactor + (timeFactorDist(randomEngine) - timeFactor)*phaseChaos
					: timeFactor + (maxCleanStretch - timeFactor)*(-phaseChaos);
				if (phaseBlur > 0) base += blurDist(randomEngine);
				return base;
			}
			if (phaseBlur > 0) {
				Sample centre = (phaseChaos < 0)
					? timeFactor + (maxCleanStretch - timeFactor)*(-phaseChaos)
					: timeFactor;
				return centre + blurDist(randomEngine);
			}
			return timeFactor; // stock
		};
		...
		if (step < splitMainPrediction) {
			...
			for (int b = startB; b < endB; ++b) {
				...
				if (b > 0) {
					// [ScoopyLoops local patch] phaseChaos/phaseBlur: see binTimeFactorFor().
					Sample binTimeFactor = binTimeFactorFor();
	...
	void findPeaks() {
		...
		// [ScoopyLoops local patch] see setFreqMapLowFill(): make the sub-lowest-peak
		// region follow the custom map by appending octave-spaced synthetic anchor peaks
		// below the lowest detected one (updateOutputMap then interpolates through them
		// instead of applying its rigid bottom translate). Allocation-free in practice
		// (<= ~12 anchors vs the bands/2 reserve). Outputs stay ascending as long as the
		// custom map is monotone; equal (clamp-plateau) outputs collapse to empty
		// interpolation ranges in updateOutputMap, which is safe.
		if (freqMapLowFill && customFreqMap && !peaks.empty() && peaks[0].input > 1) {
			size_t firstAnchor = peaks.size();
			for (Sample b = peaks[0].input*Sample(0.5); b >= 1; b *= Sample(0.5)) {
				peaks.push_back(Peak{b, freqToBand(mapFreq(bandToFreq(b)))});
			}
			std::reverse(peaks.begin() + firstAnchor, peaks.end());          // ascending
			std::rotate(peaks.begin(), peaks.begin() + firstAnchor, peaks.end()); // anchors first
		}
	}
