 ...
 */
export const SCHEMA_VERSION = 106; // v106: getFxSlotState REPORTS WHAT A RACK NEEDS (S4 follow-up) — name, latencyMs, editorAvailable, editorVisible. It answered identifier+state only, which is enough to REMEMBER a plugin and not enough to SHOW one: the first rack built on it displayed a truncated format-encoded identifier and an EDIT button over plugins that have no editor. Every value comes from an ABI call that already existed. // v105: ONE SCRATCH MODE, NOT TEN TECHNIQUES — `slTape` scratchStart/scratchSet drop `technique` and gain `cut` and `spread`. The technique table, its generator and the `scratch:check` gate are all RETIRED: the named figures (baby, flare, crab, transformer…) were never separate mechanisms, only different amounts of the same two things — how many times the fader moves within a stroke, and which way it rests — so `cut` is that axis and everything BETWEEN the old presets is now reachable. Choosing among ten fixed figures was itself most of why it sounded canned. `spread` is the timing axis the sources ask for: IOIs cluster on 1/32, 1/16 and 1/8 with two constant-duration trends measured rather than one, so a performance steps between divisions instead of sitting on one. Also fixes the defect that made every fader-based figure unusable — click width now derives from the measured 59% ARTICULATION (sounding time / IOI) instead of an absolute per-technique millisecond value applied against the whole stroke, which had stacked four closures into a 96%-silent crab and a 100%-silent transformer. // v104: slDevices REPORTS OUTPUTS (S5) — `outputDevices` + `outputChannelCount`. `activeOutputChannelCount` existed on the device layer all along and nothing reported it, so no UI could know whether an interface even HAS a pair 3/4 to route to. The donor gates its output picker on exactly this. // v103: SCRATCH PATTERNS STOP BEING STALE — `slTape` scratchStart/scratchSet gain `vary`. One identical stroke repeated is not what anyone plays, and the sources say so directly: "the actual improvising does not necessarily turn out to be a series of perfectly performed basic techniques". `vary` drives three MEASURED behaviours from one control — the TWIN-PEAKS alternation (a big excursion then one 0.585 its size; the figure took "almost one third of the performance in time" and recurred eight times "with striking similarity"), ±30% span jitter from the measured peak ranges, and thinning the crossfader off the big stroke because the measurement puts more attacks on the second peak. Period is deliberately untouched: what was measured is that "the record speed and gesture span varies, but the DURATION is constant", so the figure never drifts off the grid. Generated from published statistics; no recorded gesture data was obtained or transcribed. // v102: THE WAVEFORM IS THE RECORD — `slTape` gains `scratchFader`, and `scratchStart`/`scratchStop` grow meanings for two params they already had. A scratch is now LAUNCHED at a point: `frame` on scratchStart is where the hand landed, and if the playhead is elsewhere the engine spins the record there first, fast and audibly (up to ±16×, its own clamp — never the hand-scrub ±4), the way a hand drags a deck to the section it wants. `mode` on scratchStop is the release pair pd-scrub-engine.md §5 designed and shipped neither of: 0 = RESUME (the record plays on from where the stroke ended — no region-entry snap, one-shot from here if the tape was idle) · 1 = HOLD (latch, the old behaviour). `scratchFader` is the PLAYER's own hand on the crossfader, multiplying the technique's gate rather than replacing it, shaped engine-side by the measured battle curve. // v101: SCRATCH REACHES THE PAGE — `slTape` gains scratchStart/scratchSet/scratchStop/scratchTempo, the door for the tempo-locked turntablism patterns generated per sample in the engine (docs/specs/scratching.md). `scratchSet` is a separate verb from `scratchStart` because retuning while HELD must not re-seed the phase, or every knob movement restarts the figure; `scratchStop` completes the current stroke before releasing, because reversals land at the extrema and strokes are whole units. `technique` is a POSITION in the table generated from slengine/scratch-techniques.json — the index is the wire and both sides are emitted from that one authority, gated by `scratch:check`. `span: 0` is FADER-ONLY: the record hand moves nothing and only the crossfader chops, which is how a transformer is played over a normally playing loop. // v100: A HOST SAYS WHETHER IT HAS TAPES — `getCapabilities` gains `tape`. The looper was carved out of `PluginTapePanel` into `blocks/TapeRow.tsx` (D-SL-STUDIO-01 L1: a face composes blocks and never rebuilds one) so Studio's bottom row and ScoopyTape mount ONE implementation. That block now ships in a bundle the browser companion also loads, and the companion's WASM engine has no tape at all — so the block needs to ask rather than assume, and L1 is explicit that availability is `getCapabilities` and never "which face am I in". Inert with a stated reason, not broken. A capability rather than a probe because the alternative is discovering it by watching the first `slTape` call throw. // v99: MIDI CLOCK OUT (S9) — `midiClock` puts 24-PPQN clock, START/STOP and a live tempo on the wire. TS owns the tempo and the transport (the house law), so both are PUSHED down rather than read from a second authority that would disagree mid-ramp. Notes are NOT here: they need per-track world fields whose table is generated from a pinned source, and that edit is separate by design. // v98: TOPOLOGY UNDO (D-SL-UNDO-01) — `topologyUndo` lets the grid hand a popped topology entry to whoever owns the document. ⌘Z after adding a track did NOTHING: `undoStore` is per-track and pattern-shaped, so an add-track could only push a `swift` marker, and that marker delegates to `swiftUndo` — a method no host in this app answers. The entry carries the whole PATTERN rather than a track index, because appending a row touches all eight sections plus baseSettings.trackSettings. bpm/masterVolume/loadSample stay outside undo and are written down as such. // v97: NINE DEAD VERBS RETIRED (B1-RETIRE) — `transportGlobalPlay/Stop/Restart`, `transportDeck`, `deckSection`, `djSetting`, `djBrowser` and `toggleDjMode` leave the protocol. They were declared here and answered by NOBODY: not SlDispatch, not MergedApp, not browserLink. Their only callers were `TransportPanel` and `DjPanel`, a complete DJ surface where every control did nothing, both now deleted. What replaced them is not a rename but a different shape — deck transport is the companion's (it owns the pattern document) with `slDeck` for the engine's half, the DJ view is the expanded deck tile rather than a global mode (D-SL-DECKFULL-01), and the launch quantum is `launchQuantum` on the map with a per-strip reference (D-SL-QUANTUM-01). `menuTransport` is deliberately NOT in this list: B1/4a gave it a real answerer, so Space finally starts a deck. // v96: A PLUGIN CAN BE REMEMBERED (P6-5a) — `getFxSlotState` reports per return the identifier a plugin can be found by again (JUCE's createIdentifierString, NOT the display name — a name collides across formats and vendors) plus its opaque state; `selectFxPlugin` gains an optional `state` applied AT INSTANTIATION, so a restored plugin is never briefly heard at its defaults. A read COMMAND rather than a toolbar field because a blob runs to hundreds of KB. A blob the plugin rejects leaves it loaded at defaults: a map that remembers wrong beats a map that will not open. Document side is P6-5b. // v95: THE PLUGIN EDITOR IS REACHABLE (P6-4) — `fxSlot` is finally ANSWERED (the method existed in this file and no host handled it): op `toggleEditor` opens/hides a return's plugin editor window, and the toolbar push's `editorVisible` stops being a hardcoded false while a new optional `editorAvailable` says whether the plugin has an editor at all. Both are read from the ENGINE, never echoed from the request: the window can be closed by its own close box and "show" is a no-op on a plugin with no editor, so an echoing UI lights a lamp over nothing. The other three fxSlot ops are refused BY NAME until the mixer strip lands (P7-MIX-0). // v94: DECK OUT AS A ROUTE SOURCE (P3.5-E3) — slRoute srcKind gains 4 = deckOut, a GRID DECK's dry stereo output (index = deck, not channel). Closes a defect against shipped P3-R3: a grid deck's channel is a projection with an EMPTY bus (the core owns that deck's gain stage and already summed it), so the looper that gesture spawned was patched from silence. The deck is now named directly rather than making channelOut mean two things. // v93: PER-STRIP DRV (P3-X2) — slChannel gains setDrive (curve 0 soft · 1 tanh · 2 hard · 3 fold; amount [1,32], 1 = off), the core's pre-sum deck drive stage at the strip channel tier: post-element PRE-level, so character stays constant while fading. Tape/input strips only — a grid strip's DRV writes the session DOCUMENT (masterClipperDrive/Curve), because the core's per-deck stage is document-fed and a live projection would be stomped by the next republish. // v92: CARVE LANDS (P3-U7) — slFiles gains the read-only "/takes" mount, so a kit sample's filePath can reference the recorder's WAV directly (carve's invariant: a grid track and a tape share ONE take, no copy). Writes/removes under /takes are refused — the take library has its own verbs. // v91: OVERDUB MADE WHOLE (P3-U3) — slTape/overdubStart gains the record-source args + sourceDesc/bpmAtStart and BRACKETS A TAKE (the RAM mix is destructive; each pass persists as its own crash-safe stamped file, D-WZ-OVERDUB-01); overdubStop closes it and returns the pass's path. Before this, a punch through the dispatcher layered SILENCE (no source set) and persisted NOTHING (no take bracket). v90: TAPE timeStretch (P3-2b-5) — slTape gains setTempoMode (0 timePitch · 1 timeStretch); the engine's per-tape Signalsmith stretcher holds pitch while the timeline runs at the sync ratio, the varispeed reader staging its input (TAPE-STRETCH.md). v89: TAPE TEMPO IDENTITY, document side (P3-2b-1) — slRecord/start gains optional `bpmAtStart` (the MASTER tempo when capture began, stamped into the take's sidecar by the host; the datum a recorded loop needs to state its own bpm and sync later, MAP-SCHEMA "dual stamps"). Sidecar field is OMITTED when the caller had no tempo, so old sidecars and tempo-less ones parse identically. v88: the SPLIT TAP (merge P2-5) — slChannel/setMonitor puts sl_channel_set_monitor on the wire and one appended HotFrame scalar (slChanMonitorMask) reports the engine's own state, because the engine opens the switch at record-start and closes it at the Law C-3 handoff (D-WZ-MON-01/02). Fixes a strip arriving with its input permanently patched and audible, where the only control that stopped the feedback (`M`) also silenced the tape. v87: the plane (merge P2 step 4) — slChannel/slTape/slRoute/slRouteList/slRecord/slTakes put the merged engine's strip surface (sl_channel_*/sl_tape_*/sl_route_*) on the wire, plus 42 appended HotFrame scalars (per-channel peaks, tape playhead/state/cap, the watchdog lamp). Answered by WizardMerged only; ScoopyLoops.app refuses them like any unimplemented method. v86: shared-envelope convergence (shared/ROLLOUT.md phase 5 / merge P0-A) — command replies carry `ok` ({id, ok, result?, error?}, mirrors shared/protocol/envelope.ts), every schema object is `.strict()`. v85: MOD-11 "SHAPES" — ModChannelState gains warp/curve/fold/quant/chaos macros + stageCount/stageLevels/stageGlide step layer.
...
export const Capabilities = z.object({
  ...
  returnFx: z.boolean(),
  /** This host has TAPES — a recorder and a looper (`sl_tape_*`). False in the browser, whose WASM
   *  engine has none at all (D-SL-COMPANION-01 names the ceiling: "no tape, no recording, one
   *  deck"). It exists because `blocks/TapeRow.tsx` is mounted by faces that ship in both tiers,
   *  and L1 is explicit that availability is what `getCapabilities` answers and NEVER "which face
   *  am I in" — so the block renders inert with a stated reason instead of throwing on the first
   *  `slTape` call and looking broken. */
  tape: z.boolean(),
}).strict();
...
export const COMMANDS = {
  ...
  // MIDI CLOCK OUT (S9). Ops rather than four verbs, because they share one
  // destination and one running state and splitting them would let a caller
  // set a tempo on a clock that was never opened.
  //
  // `start` sends 0xFA (or 0xFB to CONTINUE), `stop` sends 0xFC, `tempo`
  // changes the tick rate of a RUNNING clock and is deliberately inert when
  // stopped — a tempo edit is a preference, not a transport command, and
  // launching someone's external sequencer because they dragged the master
  // tempo would be the worst reading of that gesture.
  midiClock: {
    params: z.object({
      op: z.enum(["start", "continue", "stop", "tempo"]),
      bpm: z.number().optional(),
    }).strict(),
    result: z.object({ running: z.boolean(), open: z.boolean() }).strict(),
  },
  setMidiEnabled: {
  ...
  selectFxPlugin: {
    params: z.object({
      ...
      /** P6-5: the plugin's own opaque state, applied AT INSTANTIATION so a
          restored plugin is never briefly heard at its defaults. Only a map
          restore has one; an ordinary pick from the list omits it. A blob the
          plugin rejects leaves it loaded at defaults rather than failing the
          load — a map that remembers wrong beats a map that will not open. */
      state: z.string().optional(),
    }).strict(),
    result: z.object({}).strict(),
  },
  /** What a MAP must store about the FX returns (P6-5): per return, the
      identifier the plugin can be found by again plus its opaque state.

      A COMMAND rather than a field on the `toolbar` push, because a state blob
      runs to hundreds of KB for a plugin carrying an impulse response or sample
      content — that has no business crossing the bridge twice a second for
      something no UI displays. Pulled once, when a map is saved. */
  getFxSlotState: {
    params: z.object({}).strict(),
    result: z.object({
      slots: z.array(z.object({
        returnIndex: z.number().int().min(1).max(4),
        /** null = nothing loaded in that slot, which is a fact the document
            records rather than a key it omits. */
        identifier: z.string().nullable(),
        /** null = loaded, but the plugin saves no state of its own. The
            identifier alone still restores it. */
        state: z.string().nullable(),
        /** The DISPLAY name. The identifier is a format-encoded string and is
            not something to put in front of a person. null = empty slot. */
        name: z.string().nullable(),
        /** What this return adds to the signal path, for the latency readout
            the donor's slot carries. */
        latencyMs: z.number(),
        /** Does the loaded plugin HAVE an editor? Without this an EDIT button
            gets drawn over plugins that open nothing. */
        editorAvailable: z.boolean(),
        /** Read from the ENGINE, never echoed from a request: the window can be
            closed by its own close box, so a UI trusting its last request
            lights a lamp over a window that is gone. */
  ...
  /**
   * Replay a TOPOLOGY undo/redo — restore a deck's whole pattern (D-SL-UNDO-01).
   *
   * The grid pops the entry (its undo stack is the ordered one) and hands the
   * payload to whoever owns the document; in this app that is the companion.
   * ⚠️ It carries the PATTERN rather than a track index because appending a row
   * touches all eight sections plus `baseSettings.trackSettings` — an index
   * would make the receiver re-implement the mutator backwards.
   */
  topologyUndo: {
    params: z
      .object({ deck: z.number().int().min(0).max(2), pattern: z.unknown() })
      .strict(),
  ...
  slChannel: {
    params: z.object({
      action: z.enum(["setSource", "setLevel", "setMute", "setSend", "setMonitor", "setDrive"]),
      ...
      /** setDrive: the per-strip DRV stage (P3-X2) — 0 soft · 1 tanh · 2 hard ·
          3 fold, the core's own MasterDriveCurve. */
      curve: z.number().int().min(0).max(3).optional(),
      /** setDrive: always-on input gain into a 0 dBFS ceiling, applied
          post-element PRE-level (the deck stage's topology). 1 = off, and off
          is a bypass branch, not a unity multiply — the bit-exact identity
          path survives an untouched strip. */
      amount: z.number().min(1).max(32).optional(),
    }).strict(),
  ...
  slTape: {
    params: z.object({
      action: z.enum([
        ...
        /** P3-2b-5: mode 0 timePitch · 1 timeStretch — what the rate DRIVES.
            timeStretch lazily allocates the tape's stretcher (async warm-up;
            the render stays dry until warm, which the latency policy absorbs). */
        "setTempoMode",
        "scrubBegin", "scrubTo", "scrubEnd",
        /** TURNTABLISM SCRATCH (v101, docs/specs/scratching.md). A tempo-locked
            pattern that runs for as long as it is HELD, generated per sample in
            the engine and driving the same scrub path a finger does — which is
            the point, since the 10 ms rate one-pole is what produces the phantom
            click. `scratchSet` is separate from `scratchStart` because retuning
            while held must not re-seed the phase; `scratchStop` completes the
            current stroke before releasing; `scratchTempo` is ENGINE-WIDE and
            ignores `tape`. */
        "scratchStart", "scratchSet", "scratchStop", "scratchTempo", "scratchFader",
      ...
      mode: z.number().int().min(0).max(3).optional(), // trigger / overdubStart · scratchStop: 0 resume (play out) · 1 hold
      frame: z.number().min(0).optional(), // seek / scrubTo (fractional: scrub) · scratchStart: WHERE THE HAND LANDED, and the record spins there before the pattern begins
      ...
      /** scratchStart/Set: HOW MUCH CROSSFADER, 0..1. ⚠️ This replaced a
          `technique` index and the ten-figure table behind it. The named figures
          were never separate mechanisms — only different amounts of the same two
          things, how many times the fader moves within a stroke and which way it
          rests — so this is that axis, and everything between the old presets is
          now reachable. 0 = the pure record hand (baby, scribble); rising adds
          clicks and then spreads them to both strokes; past ~0.85 the fader
          rests CLOSED (transformer, chirp). */
      cut: z.number().min(0).max(1).optional(),
      /** scratchStart/Set: HOW MUCH THE TIMING MOVES, 0..1. IOIs cluster on
          1/32, 1/16 and 1/8 and the measurement found two constant-duration
          trends rather than one, so real playing steps BETWEEN divisions. Each
          cycle may step one either way — always on the grid, never between. */
      spread: z.number().min(0).max(1).optional(),
      /** scratchStart/Set: one full back-and-forth, in BEATS — musical
          divisions, never milliseconds, because crossfader IOIs cluster on 1/32,
          1/16 and 1/8. */
      periodBeats: z.number().positive().optional(),
      /** scratchStart/Set: fraction of the loop the gesture covers; measured
          spans are SMALL. ⚠️ 0 is FADER-ONLY — the record hand moves nothing and
          only the crossfader chops, which is how a transformer is played over a
          normally playing loop. */
      span: z.number().min(0).max(1).optional(),
      /** scratchTempo: the tempo patterns lock to. Engine-wide, so `tape` is
          ignored — a pattern locks to THE tempo. */
      bpm: z.number().positive().optional(),
      /** scratchStart/Set: HOW MUCH THE FIGURE VARIES, 0..1. Zero means every
          stroke identical, which is nobody's playing — the sources are blunt
          that improvising is not "a series of perfectly performed basic
          techniques". Above zero the engine drives three MEASURED behaviours at
          once: the twin-peaks big/small alternation (the second excursion 0.585
          of the first — a figure that took "almost one third of the performance
          in time"), ±30% span jitter, and thinning the crossfader off the big
          stroke. Period is untouched on purpose: what was measured is that span
          varies while DURATION stays constant. */
      vary: z.number().min(0).max(1).optional(),
      /** scratchFader: the PLAYER's own hand on the crossfader, 0 shut .. 1
          open, resting open. Multiplies the technique's gate rather than
          replacing it, and is shaped engine-side by the measured battle curve —
          a narrow transition with dead ground either side, never linearly. */
  ...
  slRoute: {
    params: z.object({
      ...
      /** add/wouldCycle source: 0 channelOut · 1 channelSend · 2 deviceInput
          · 3 fxReturn · 4 deckOut (a GRID DECK's dry output — index is a deck,
          not a channel; P3.5-E3). Encodings are mapApply's = sl_engine.h's. */
      srcKind: z.number().int().min(0).max(4).optional(),
  ...
  slDevices: {
    ...
    result: z.object({
      ...
      /** Every OUTPUT device this host could switch to. */
      outputDevices: z.array(z.string()).optional(),
      /** How many output channels the active output device actually has (S5).
          The gate on offering a pair at all: the donor publishes its output
          picker only when this exceeds 2, because a picker offering channels
          the hardware does not have is a control that reaches nothing. */
      outputChannelCount: z.number().int().optional(),
...
export const FxSlotState = z.object({
  ...
  /** Is the plugin's editor window ON SCREEN? THE WINDOW, not the last request
      (P6-4) — the user can close it by its own close box, and a "show" on a
      plugin with no editor does nothing, so a UI that echoed its own click would
      light this lamp over nothing. Display it; never assume it. */
  editorVisible: z.boolean(),
  /** Does this slot have a plugin WITH an editor of its own (P6-4)? Optional
      because the desktop's toolbar push predates it; absent reads as "unknown",
      and a UI should fall back to `pluginName !== null` rather than hiding the
      door. */
